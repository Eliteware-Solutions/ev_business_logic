...
class SuccessResult<T> extends Result<T> {
  ...
  const SuccessResult({this.data, this.message}) : super.private();
}

class FailureResult<T> extends Result<T> {
  final dynamic error;
  final T? data;
  const FailureResult({required this.error, this.data}) : super.private();
}

abstract class RepoResult<T> extends Result<T> {
  const RepoResult._() : super.private();
